...
class SetSendingForm(StyleFormMixin, forms.ModelForm):
    ...
    class Meta:
        model = SetSending
        fields = ['time_start', 'time_end', 'interval', 'status', 'message']


class AddSending(forms.Form):
    time_start = forms.DateTimeField(widget=forms.DateTimeInput(attrs={'class': 'form-control'}), label='Время начала рассылки')
